...
var (
	supportedDHKEXGroupsOnce sync.Once
	supportedDHKEXGroupsList []dhKEXGroup
)
...
func supportedDHKEXGroups() []dhKEXGroup {
	supportedDHKEXGroupsOnce.Do(func() {
		specs := []struct {
			size int
			hex  string
		}{
		...
		supportedDHKEXGroupsList = make([]dhKEXGroup, 0, len(specs))
		for _, s := range specs {
			p, _ := new(big.Int).SetString(s.hex, 16)
			supportedDHKEXGroupsList = append(supportedDHKEXGroupsList, dhKEXGroup{size: s.size, p: p})
		}
	})
	return supportedDHKEXGroupsList
}
